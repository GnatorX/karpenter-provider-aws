...
| karpenter.k8s.aws/instance-suffix                              | d                    | [AWS Specific] Instance type suffix characters indicating features: `d` (NVMe storage), `n` (network optimized), `z` (high frequency), `e` (extra capacity), `b` (EBS optimized). Excludes `a`/`i`/`g` which are covered by other labels. |
